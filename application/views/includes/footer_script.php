	...
	<script type="text/javascript">
		...
		$(document).on('click','#savebuyeritem',function(e){
			...
			$(".loader_ajax").show();
			   ...
			   var item_remark_box =   $('#item_remark_box').val();
		...
		$(document).on('click','.editbuyerpoitem',function(e){  
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR)
				{
						...
						$('#item_remark_box').val(fetchResponse.item_remark_box); 
								...
								success: function(data, textStatus, jqXHR)
								{
									...
									if(obj.status=='success'){
										swal({
											...
											$.ajax({
											url : "<?php echo base_url();?>checkifsupplieridisinsupplierpoconfirm",
											type: "POST",
											data : 'id='+elemF.attr('data-id'),
											success: function(data, textStatus, jqXHR)
											{
												...
												if(obj.status=='success'){
													swal({
														title: "Already in use!",
														text: "This Supplier po is linked to Supplier PO Confirmation",
														//type: "danger",
														icon: "success",
														button: "Ok",
														},function(){ 
	...
	<script type="text/javascript">
		...
		$(document).on('click','#saveBillofmaterialtem',function(e){
			...
			$(".loader_ajax").show();
			   ...
			   var bom_status_item =  $('#bom_status_item').val();
...
<?php if($pageTitle=='Vendor Bill of Material' || $pageTitle=="Add New Vendor Bill Of Material" || $pageTitle=="Edit Vendor Bill Of Material" || $pageTitle=="View Vendor Bill Of Material" ){ ?>
	<script type="text/javascript">
		$(document).ready(function() {
	...
	<script type="text/javascript">
		...
		$(document).on('click','#saveincomingitem',function(e){
			...
			$(".loader_ajax").show();
			   ...
			   var additional_process =   $('#additional_process').val();
			   var qc_person_name =   $('#qc_person_name').val();
			   var lr_no =   $('#lr_no').val();

               var incoming_item_status =   $('#incoming_item_status').val();
			   ...
			   var pre_additional_process  =   $('#additional_process').val();
			   var pre_remark  =   $('#remark').val();
			   ...
			   var cleaning_required =   $('#cleaning_required').val();
...
<?php if($pageTitle=='Add New Packing Instaruction' || $pageTitle=='Packing Instaruction'  || $pageTitle=='Add Packing Instraction Details' || $pageTitle=='Edit Packing Instraction Details' || $pageTitle=='Clone Packing Instraction Item Details' ){ ?>
	<script type="text/javascript">
		...
		$(document).on('click','.clonepackgingitemdetails',function(e){
			var elemF = $(this);
			e.preventDefault();
			swal({
				title: "Are you sure?",
				...
				if (isConfirm) {
							$.ajax({
								url : "<?php echo base_url();?>admin/clonepackgingitemdetails",
								type: "POST",
								data : 'id='+elemF.attr('data-id'),
								success: function(data, textStatus, jqXHR)
								{
...
<?php if($pageTitle=='Search Stock'){ ?>
	<script type="text/javascript">    
		...
		$(document).on('click','#update_stock',function(e){
					...
					success: function(data, textStatus, jqXHR)
					{
						...
						else
						{
								swal({
									...
									window.location.href = "<?php echo base_url().'admin/searchstock/'?>"+stock_id+"/"+finishgood_id;
							    });		
						}
					},
					error: function (jqXHR, textStatus, errorThrown)
			...
			$(document).on('click','#forbalanceupdatestockform',function(e){
			e.preventDefault();
			$(".loader_ajax").show();
		    var finishgood_id = $('#finishgood_id').val();
			var balence_qty_in_pcs = $('#balence_qty_in_pcs').val();
			var stock_id = $('#stock_id').val();
			$.ajax({
					url : "<?php echo ADMIN_PATH;?>admin/forbalanceupdatestockform",
					type: "POST",
					 data : {'finishgood_id' : finishgood_id,'balence_qty_in_pcs':balence_qty_in_pcs,'stock_id':stock_id},
					success: function(data, textStatus, jqXHR)
					{
...
<?php if($pageTitle=='Payment Details Report'){ ?>
	<script type="text/javascript">
		...
		function getallPatmentdetailsreport(vendor_supplier_name,vendor_name,supplier_name,payment_details_no,status){
			...
			var dt = $('#view_payment_details_report').DataTable({
	            ...
	            "serverSide": true,
	            "ajax":{
					url :"<?php echo base_url();?>admin/fetchPaymentdetailsreport/"+vendor_supplier_name+"/"+vendor_name+"/"+supplier_name+"/"+payment_details_no+"/"+status,
                    type: "post",
	            },
	        });
        }
...
<?php if($pageTitle=='Scarp Invoice' || $pageTitle=='Add New Scrap Invoice' || $pageTitle=='Edit Scrap Invoice'){ ?>
	<script type="text/javascript">
			...
			$(document).on('click','.deletescrapinvoicedata',function(e){
				var elemF = $(this);
				e.preventDefault();

				swal({
					title: "Are you sure?",
					text: "Delete Scrap Invoice",
					type: "warning",
					showCancelButton: true,
					closeOnClickOutside: false,
					confirmButtonClass: "btn-sm btn-danger",
					...
					if (isConfirm) {
								$.ajax({
									url : "<?php echo base_url();?>deletescrapinvoicedata",
									type: "POST",
									data : 'id='+elemF.attr('data-id'),
									success: function(data, textStatus, jqXHR)
									{
										...
										if(obj.status=='success'){
											swal({
												title: "Deleted!",
												text: "Scrap Invoice Details Succesfully Deleted",
												icon: "success",
												button: "Ok",
												},function(){ 
													window.location.href = "<?php echo base_url()?>scrap_invoice";
					...
					swal("Cancelled", "Scrap Invoice deletion cancelled ", "error");
					}
				});
	        });
			...
			$(document).on('click','.deleteScrapinvoiceitem',function(e){
				...
				swal({
					title: "Are you sure?",
					text: "Delete Scrap Invoice Item",
					type: "warning",
					showCancelButton: true,
					closeOnClickOutside: false,
					confirmButtonClass: "btn-sm btn-danger",
					...
					if (isConfirm) {
								$.ajax({
									url : "<?php echo base_url();?>deleteScrapinvoiceitem",
									type: "POST",
									data : 'id='+elemF.attr('data-id'),
									success: function(data, textStatus, jqXHR)
									{
										...
										if(obj.status=='success'){
											swal({
												title: "Deleted!",
												text: "Scrap Invoice Item Succesfully Deleted",
												icon: "success",
												button: "Ok",
												},function(){ 
													if(scrap_invoice_id_main){
					...
					swal("Cancelled", "Scrap Invoice Item deletion cancelled ", "error");
					}
				});
	        });
...
<?php if($pageTitle=='Packing Challan' || $pageTitle=='Add New Packing Challan' || $pageTitle=='Edit Packing Challan'){ ?>
	<script type="text/javascript">
			...
			$(document).on('change', '#quantity_in_gonis,#rate', function(){	
				
			    $("#quantity_in_gonis").val();
				$("#rate").val();

				 if($("#quantity_in_gonis").val()){
					 var quantity_in_gonis = $("#quantity_in_gonis").val();
				 }else{
					 var quantity_in_gonis = 0;
				 }
				 ...
				 var total_amount = parseFloat(quantity_in_gonis) *  parseFloat(rate);
			...
			$(document).on('click','.deletepackinchallandata',function(e){
				...
				swal({
					...
					if (isConfirm) {
								$.ajax({
									url : "<?php echo base_url();?>deletepackinchallandata",
									type: "POST",
									data : 'id='+elemF.attr('data-id'),
									success: function(data, textStatus, jqXHR)
									{
										...
										if(obj.status=='success'){
											swal({
												title: "Deleted!",
												text: "Packing Challan Succesfully Deleted",
												icon: "success",
												button: "Ok",
												},function(){ 
					...
					swal("Cancelled", "Packing Challan Data deletion cancelled ", "error");
					}
				});
	        });
			...
			$(document).on('click','.deletepackingchallanitem',function(e){
				...
				swal({
					title: "Are you sure?",
					text: "Delete Packing Challan Item",
					type: "warning",
					showCancelButton: true,
					closeOnClickOutside: false,
					confirmButtonClass: "btn-sm btn-danger",
					...
					if (isConfirm) {
								$.ajax({
									url : "<?php echo base_url();?>deletepackingchallanitem",
									type: "POST",
									data : 'id='+elemF.attr('data-id'),
									success: function(data, textStatus, jqXHR)
									{
										...
										if(obj.status=='success'){
											swal({
												title: "Deleted!",
												text: "Packing Challan Item Succesfully Deleted",
												icon: "success",
												button: "Ok",
												},function(){ 
...
<?php if($pageTitle=='Add New TDIR' || $pageTitle=='TDIR Report' ||  $pageTitle=='Edit TDIR' ||  $pageTitle=='Incoming Lots Data' || $pageTitle=='TDIR Attachment'){ ?>
 <script type="text/javascript">
				...
				$(document).on('change','.vendor_po_number_for_vendor_po_date',function(e){  
					...
					//$(".loader_ajax").show();
					// $("#customers-list").html('');
					var vendor_po_id = $('#vendor_po_number').val();
					$.ajax({
						url : "<?php echo ADMIN_PATH;?>get_vendorpodata",
						type: "POST",
						data : {'vendor_po_id' : vendor_po_id},
...
<?php if($pageTitle=='Instrument Master Detail Data'){ ?>
	<script type="text/javascript">
		...
		$('#instrumentForm').on('submit', function(e){
			...
			$.ajax({
				url: '<?php echo base_url();?>admin/saveinstrumentdetailsdata',
				type: 'POST',
				data: $(this).serialize(),
				dataType: 'json',
				success: function(res){
				if(res.status){
					// Swal.fire('Success', res.message, 'success');
					$('#instrumentdetailsModal').hide('show');
            ...
            <script type="text/javascript">
				$(document).ready(function() {
					...
					var dt = $('#rework_record_incoming_item_list').DataTable({
						...
						"serverSide": true,
						"ajax":{
							url :"<?php echo base_url();?>admin/fetchreworkrecordincomingdetailslist/"+vendor_po+"/"+part_no+"/"+rework_id,
							type: "post",
						},
					});
				...
				$(document).on('click','.addreworkreasondata',function(e){
					var elemF = $(this);
			        var incoming_item_data_id = elemF.attr('data-id');
					var rework_id = elemF.attr('rework_id');
					$('#incoming_item_data_id').val(incoming_item_data_id);
					
				});


				$(document).on('click','#saverejectionreworkitemdata',function(e){
					e.preventDefault();
					$(".loader_ajax").show();

					var rework_id = $('#rework_id').val();

					var formData = new FormData($("#saverejectionreworkitemdataform")[0]);
							$.ajax({
									...
									else if(fetchResponse.status == 'success')
									{
										swal({
											...
											button: "Ok",
												...
												window.location.href = "<?php echo base_url().'reworkrecordlotnumberrecord/'?>"+rework_id;
										});		
									}
...
<?php if ($pageTitle == 'Add Instrument Store Form') { ?>
	<script type="text/javascript">
		...
		$(document).on("click", ".removeaddrejectionitemdata", function() {
			...
			$("#sampling_trans_id").val($(this).data("sampling_trans_id"));
			$("#qty_popup").val($(this).data("qty"));
			$("#instrument_id_removed").val($(this).data("instrument_id"));
			$("#type").val($(this).data("type"));
			$("#grade").val($(this).data("grade"));
			$("#unit").val($(this).data("unit"));
			$("#class_1").val($(this).data("class_1"));
		...
		$(document).on('click','#editstoreformqtyremovedata',function(e){
            ...
            var instrument_id_removed = $('#instrument_id_removed').val();

			
			$.ajax({
				url: "<?php echo base_url('editstoreformqtyremovedata'); ?>",
				type: "POST",
				data: {"ticket_no_popup": ticket_no_popup,"instrument_name_popup":instrument_name_popup, "measuring_size_popup":measuring_size_popup,"qty_popup":qty_popup, "qty_assign":qty_assign, "qty_remark":qty_remark, "qty_removed": qty_removed,'sampling_id':sampling_id,'sampling_trans_id':sampling_trans_id,"type":type,"grade":grade,"unit":unit,"class_1":class_1,"part_number":part_number,"certificateremove":certificateremove,"status":status,'instrument_id_removed':instrument_id_removed},
				dataType: "json",
                cache:false,
				success: function(fetchResponse, textStatus, jqXHR)
				{
	...
	<script type="text/javascript">
	            $(document).ready(function() {
						...
						if(from_date){
						   ...
						   var form_date_val = 'NA';
						}

						if(to_date){
						   var to_date_val = to_date;
						}else{
						   var to_date_val = 'NA';
						}

				
						getRmcertificateList(vendor_supplier_name,vendor_name,supplier_name,vendor_po_number,supplier_po_number,status,form_date_val,to_date_val);
				...
				function getRmcertificateList(vendor_supplier_name,vendor_name,supplier_name,vendor_po_number,supplier_po_number,status,form_date_val,to_date_val){
				   ...
				   $("#view_rm_certificate_report").dataTable().fnDestroy();
					...
					var dt = $('#view_rm_certificate_report').DataTable({
							...
							"serverSide": true,
							"ajax":{
								url :"<?php echo base_url();?>admin/fetchrmcertificatelist/"+vendor_supplier_name+"/"+vendor_name+"/"+supplier_name+"/"+vendor_po_number+"/"+supplier_po_number+"/"+status+"/"+form_date_val+"/"+to_date_val,
								type: "post",
							},
						});
				}	
				...
				$(document).on('click','.updatestatusofrmcertificate',function(e){  
						...
						//$('#rmcertirmcertificatemodelficate').modal('show');
						if(elemF.attr('data-identity')=='Supplier'){
							$.ajax({
								url : "<?php echo ADMIN_PATH;?>admin/getpreviousdataforRMcertificatestatusupdate_supplier",
								type: "POST",
							...
							success: function(data, textStatus, jqXHR) {
								...
								if (fetchResponse.status == "failure") {
									...
									swal({
										...
										$("#modal-md").hide();
										window.location.href = "<?php echo base_url() . 'rmtestcertificate' ?>";
									});
								}
...
<?php if($pageTitle=='Forging Scarp Working' || $pageTitle=='Add New Forging Scarp Working' ||  $pageTitle=='Edit Forging Scarp Working' ||  $pageTitle=='Forging Scrap Report Item Dtails' || $pageTitle=='view Forging Scrap Report Item Dtails'){?>
	<script type="text/javascript">
				...
				$('#sent_rm_in_kgs_section_2').on('keyup change', function () {
					let sent_rm_in_kgs_section_1  = parseFloat($('#sent_rm_in_kgs_section_1').val()) || 0;
					let sent_rm_in_kgs_section_2 = parseFloat($('#sent_rm_in_kgs_section_2').val()) || 0;
...
<script type="text/javascript">
            ...
            $(document).on('click','#export_to_excel_forginscrap_record_details',function(e){
				e.preventDefault();
				$(".loader_ajax").show();
				...
				$.ajax({
					url : "<?php echo ADMIN_PATH;?>admin/exporttoexcelforginscraprecorddetails/"+vendor_name+"/"+forgin_report_status+"/"+from_date+"/"+to_date,
					type: "POST",
					// data : {'hospitals' : hospitals, 'driver' : driver,'ride_start':ride_start,'ride_stop':ride_stop},
					success: function(data, textStatus, jqXHR)
					{
						...
						{
							// $(".sales_tracking_report_name_error").html("");
							window.location.href = "<?php echo ADMIN_PATH;?>admin/exporttoexcelforginscraprecorddetails/"+vendor_name+"/"+forgin_report_status+"/"+from_date+"/"+to_date;
						}
					},
					error: function (jqXHR, textStatus, errorThrown)
					{
