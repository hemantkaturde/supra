...
class Admin_model extends CI_Model
{
    ...
    public function getVendorpodata($params){
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                ...
                $data[$counter]['action'] .= "<a href='".ADMIN_PATH."editVendorpo/".$value['id']."' style='cursor: pointer;'><i style='font-size: large;cursor: pointer;' class='fa fa-pencil-square-o' aria-hidden='true'></i></a>  &nbsp";
    ...
    public function getfinishedgoodsPartnumberByid($part_number,$flag){
        ...
        if($flag=='Supplier'){
            ...
            $this->db->select('*,'.TBL_RAWMATERIAL.'.net_weight as supplier_goods_net_weight,'.TBL_RAWMATERIAL.'.sac as supplier_goods_sac,'.TBL_RAWMATERIAL.'.type_of_raw_material as name,'.TBL_RAWMATERIAL.'.HSN_code as hsn_code,'.TBL_RAWMATERIAL.'.gross_weight as groass_weight');
    ...
    public function getstockformdata($params){
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                ...
                $data[$counter]['action'] .= "<a href='".ADMIN_PATH."reworkrejectionreturn' style='cursor: pointer;'><i style='font-size: x-large;cursor: pointer;' class='fa fa-ban' aria-hidden='true'></i></a>   &nbsp ";
