...
class Admin_model extends CI_Model
{
    ...
    public function getSuppliergoodsPartnumberByidvendorpoconfirmation($part_number,$flag,$vendor_po_number,$poitemid){
        ...
        if($flag=='Supplier'){
            ...
            $this->db->where(TBL_FINISHED_GOODS.'.raw_id',$part_number);
            $query = $this->db->get(TBL_FINISHED_GOODS);
            $data = $query->result_array();
            return $data;
        }
       ...
       // $this->db->group_by(TBL_BUYER_PO_MASTER_ITEM.'.part_number_id');
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                ...
                $get_export_invoice_details =$this->getexportinvoicedetails($value['buyer_po_idpo'],$value['part_id'],$value['buyer_po_part_delivery_date']);
    ...
    public function exportbuyerdetailsrecord($buyer_name,$part_number,$from_date,$to_date){
        ...
        if(count($fetch_result) > 0)
        {
            foreach ($fetch_result as $key => $value)
            {
                ...
                $get_export_invoice_details =$this->getexportinvoicedetails($value['buyer_po_idpo'],$value['part_id'],$buyer_po_part_delivery_date);
                if($get_export_invoice_details){
                    $buyer_invoice_number = $get_export_invoice_details[0]['buyer_invoice_number'];
                    $buyer_invoice_qty = $get_export_invoice_details[0]['buyer_invoice_qty'];
                    $buyer_invoice_date = $get_export_invoice_details[0]['buyer_invoice_date'];
...
public function get_preexport_item_details($part_number,$main_expot_id,$buyer_po_id,$buyer_name_id){
    ...
    $this->db->select(TBL_FINISHED_GOODS.'.fin_id,'.TBL_FINISHED_GOODS.'.part_number,'.TBL_FINISHED_GOODS.'.name,'.TBL_BUYER_PO_MASTER.'.sales_order_number,'.TBL_BUYER_PO_MASTER.'.id as buyer_po_number_id');
    $this->db->join(TBL_FINISHED_GOODS, TBL_FINISHED_GOODS.'.fin_id = '.TBL_BUYER_PO_MASTER_ITEM.'.part_number_id');
    $this->db->join(TBL_BUYER_PO_MASTER, TBL_BUYER_PO_MASTER.'.id = '.TBL_BUYER_PO_MASTER_ITEM.'.buyer_po_id');
...
public function fetchproductionstatusreportdata($params,$vendor_name,$status,$part_number,$vendor_po){
    ...
    $this->db->select('*,'.TBL_VENDOR.'.vendor_name as vendorname,'.TBL_BILL_OF_MATERIAL.'.id as billofmaterialid,'.TBL_FINISHED_GOODS.'.part_number as partno,'.TBL_BUYER_MASTER.'.buyer_name as buyer, 2 as flag,'.TBL_BILL_OF_MATERIAL.'.bom_status,'.TBL_VENDOR_PO_MASTER_ITEM.'.order_oty as vendor_order_qty_co,'.TBL_BILL_OF_MATERIAL_ITEM.'.vendor_actual_recived_qty as vendor_received_qty_co,'.TBL_VENDOR_PO_MASTER.'.po_number as v_po_number,'.TBL_FINISHED_GOODS.'.name as part_description,'.TBL_VENDOR_PO_MASTER.'.delivery_date,'.TBL_BILL_OF_MATERIAL_ITEM.'.id as vendor_bill_item_id,"Bill Of Material" as flag,"bom" as notes_status');
    $this->db->join(TBL_VENDOR, TBL_VENDOR.'.ven_id= '.TBL_BILL_OF_MATERIAL.'.vendor_name');
    $this->db->join(TBL_VENDOR_PO_MASTER, TBL_VENDOR_PO_MASTER.'.id= '.TBL_BILL_OF_MATERIAL.'.vendor_po_number');
    $this->db->join(TBL_VENDOR_PO_MASTER_ITEM, TBL_VENDOR_PO_MASTER_ITEM.'.vendor_po_id= '.TBL_VENDOR_PO_MASTER.'.id');
    $this->db->join(TBL_BUYER_MASTER, TBL_BILL_OF_MATERIAL.'.buyer_name= '.TBL_BUYER_MASTER.'.buyer_id');
