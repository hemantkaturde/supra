...
class User_model extends CI_Model
{
    ...
    {
        $this->db->select('userId, name, email, mobile, roleId,team_id,password');
        $this->db->from('tbl_users');
        $this->db->where('isDeleted', 0);
        $this->db->where('userId', $userId);
        $query = $this->db->get();
    ...
    {
        $this->db->select('userId, name, email, mobile, roleId,password');
        $this->db->from('tbl_users');
        $this->db->where('isDeleted', 0);
        $this->db->where('userId', $userId);
        $query = $this->db->get();
    ...
    function updateCompanyInformation($company_id=NULL,$compnayInfo)
    {
        if($company_id > 0){
            $this->db->where('id', $company_id);
            $this->db->update('tbl_compnayinfo', $compnayInfo);
            return $this->db->affected_rows();
        }else{
            $this->db->trans_start();
            $this->db->insert('tbl_compnayinfo', $compnayInfo);
            $insert_id = $this->db->insert_id();
            $this->db->trans_complete();
            return $insert_id;
    ...
    public function getCompanyInformation(){
        ...
        $this->db->select('*');
        $this->db->limit(1);
        $query = $this->db->get('tbl_compnayinfo as BaseTbl');
        return $query->row();
    }
